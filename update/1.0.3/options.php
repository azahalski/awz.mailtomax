...
use Awz\Mailtomax\Access\AccessController;
use Awz\Mailtomax\Helper;
use Bitrix\Main\Security\Random;

Loc::loadMessages(__FILE__);
global $APPLICATION;
$module_id = "awz.mailtomax";
if(!Loader::includeModule($module_id)) return;
Extension::load('ui.sidepanel-content');
$request = Application::getInstance()->getContext()->getRequest();
$APPLICATION->SetTitle(Loc::getMessage('AWZ_MAILTOMAX_OPT_TITLE'));
...
$aTabs[] = array(
    "DIV" => "edit1",
    "TAB" => Loc::getMessage('AWZ_MAILTOMAX_OPT_SECT1'),
    "ICON" => "vote_settings",
    "TITLE" => Loc::getMessage('AWZ_MAILTOMAX_OPT_SECT1')
);
...
?>
    ...
    <form method="POST" action="<?=$saveUrl?>" id="FORMACTION">
            ...
            <td colspan="2">
                <div class="ui-alert ui-alert-primary">
                    <span class="ui-alert-message">
                        <?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC')?>
                    </span>
                </div>
            </td>
        </tr>
        <tr>
            <td colspan="2"><style>.awz-opts-table {border-spacing:0;}.awz-opts-table tr:nth-child(even) {background:#fff;}.awz-opts-table tr td {padding:3px;}</style><table class="awz-opts-table" style="width:100%;">
                    <tr>
                        ...
                        <th style="text-align: left;min-width:80px;"><?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC_1')?></th>
                        <th style="text-align: left;"><?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC_2')?></th>
                        <th style="text-align: left;"><?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC_3')?></th>
                        <th style="text-align: left;"><?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC_4')?></th>
                        <th style="text-align: left;"><?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SHOW_DESC_5')?></th>
                    </tr>
        <?
        ...
        $tabControl->Buttons();
        ?>
        <input <?if (!AccessController::isEditSettings()) echo "disabled" ?> type="submit" class="adm-btn-green" name="Update" value="<?=Loc::getMessage('AWZ_MAILTOMAX_OPT_L_BTN_SAVE')?>" />
        ...
        <?if(AccessController::isViewRight()){?>
            <button class="adm-header-btn adm-security-btn" onclick="BX.SidePanel.Instance.open('<?=$saveUrl?>');return false;">
                <?=Loc::getMessage('AWZ_MAILTOMAX_OPT_SECT2')?>
            </button>
        <?}?>
        <?$tabControl->End();?>
    </form>
