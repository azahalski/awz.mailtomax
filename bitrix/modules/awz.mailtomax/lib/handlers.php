...
class Handlers {
    ...
    public static function OnBeforeProlog()
    {
        ...
        if($request->get('AWZ_MAILTOMAX_HIDDEN') != 'Y') {
            return;
        }
        ...
        $active = $request->get('AWZ_MAILTOMAX_ACTIVE') === 'Y';
        $decline = $request->get('AWZ_MAILTOMAX_DECLINE') === 'Y';
    ...
    public static function OnAdminTabControlBegin(&$form)
    {
        ...
        $html = '
<tr class="heading">
    <td colspan="2">' . Loc::getMessage('AWZ_MAILTOMAX_ADMIN_TITLE') . '
    <input type="hidden" name="AWZ_MAILTOMAX_HIDDEN" value="Y">
    </td>
</tr>
<tr>
    <td style="width:40%">' . Loc::getMessage('AWZ_MAILTOMAX_ADMIN_ACTIVE') . '</td>
    <td style="width:60%">
        <input type="checkbox" value="Y" name="AWZ_MAILTOMAX_ACTIVE" ' . ($isActive ? 'checked' : '') . ' ' . (!$canEdit ? 'disabled' : '') . '>
    </td>
</tr>
<tr>
    <td style="width:40%">' . Loc::getMessage('AWZ_MAILTOMAX_ADMIN_DECLINE') . '</td>
    <td style="width:60%">
        <input type="checkbox" value="Y" name="AWZ_MAILTOMAX_DECLINE" ' . ($isDecline ? 'checked' : '') . ' ' . (!$canEdit ? 'disabled' : '') . '>
    </td>
</tr>';
        if(Option::get($moduleId, 'DISABLED', 'N', '') == 'Y'){
            $html .= '<tr>
    <td colspan="2" style="text-align: center;">
        '.Loc::getMessage('AWZ_MAILTOMAX_ADMIN_DISABLED').'
    </td>
</tr>';
        }
        ...
        $form->tabs[] = array(
            "DIV" => "awz_mailtomax_settings",
            "TAB" => Loc::getMessage('AWZ_MAILTOMAX_ADMIN_TAB'),
            "ICON" => "",
            "TITLE" => Loc::getMessage('AWZ_MAILTOMAX_ADMIN_TITLE'),
            "CONTENT" => '' . $html . ''
        );
    ...
    public static function OnBeforeEventSend(&$arFields, &$eventMessage)
    {
        ...
        )
        {
            ...
            {
                $message->compile();
                // Конвертируем HTML в текст для Макс (с сохранением поддерживаемой разметки)
