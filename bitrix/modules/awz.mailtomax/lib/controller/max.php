...
namespace Awz\Mailtomax\Controller;

use Awz\Mailtomax\Filters\HookAuth;
...
class Max extends Controller
{
    ...
    public function webhookAction($key = '')
    {
        ...
        if (empty($rawData)) {
            $this->addError(new Error('No data received from Макс'));
            return ['status' => 'error', 'message' => 'No data received'];
        }

        // Макс webhook структура: {"update_id":..., "message":{"chat":{"id":...}}}
        ...
        Option::set('awz.mailtomax', 'TGID', (string)$chatId, "");
    ...
    /**
     * Финализация ответа - вывод в формате JSON для Макс
     *
